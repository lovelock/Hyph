...
ZEPHIR_INIT_CLASS(Hyph_Hylog_Hylog) {
	...
	ZEPHIR_REGISTER_CLASS(Hyph\\Hylog, Hylog, hyph, hylog_hylog, hyph_hylog_hylog_method_entry, 0);

	zend_declare_property_string(hyph_hylog_hylog_ce, SL("path"), "", ZEND_ACC_PRIVATE TSRMLS_CC);

	zephir_declare_class_constant_string(hyph_hylog_hylog_ce, SL("HYLOG_VERSION"), "0.1.0");

	zephir_declare_class_constant_string(hyph_hylog_hylog_ce, SL("EMERGENCY"), "EMERGENCY");

	zephir_declare_class_constant_string(hyph_hylog_hylog_ce, SL("ALERT"), "ALERT");

	zephir_declare_class_constant_string(hyph_hylog_hylog_ce, SL("CRITICAL"), "CRITICAL");

	zephir_declare_class_constant_string(hyph_hylog_hylog_ce, SL("ERROR"), "ERROR");

	zephir_declare_class_constant_string(hyph_hylog_hylog_ce, SL("WARNING"), "WARNING");

	zephir_declare_class_constant_string(hyph_hylog_hylog_ce, SL("NOTICE"), "NOTICE");

	zephir_declare_class_constant_string(hyph_hylog_hylog_ce, SL("INFO"), "INFO");

	zephir_declare_class_constant_string(hyph_hylog_hylog_ce, SL("DEBUG"), "DEBUG");
...
PHP_METHOD(Hyph_Hylog_Hylog, checkPath) {
	...
	if (!((zephir_file_exists(&_0 TSRMLS_CC) == SUCCESS))) {
		...
		if (ZEPHIR_IS_FALSE_IDENTICAL(&mkdirResult)) {
			...
			zephir_check_call_status();
			zephir_throw_exception_debug(&_3$$4, "hyph/hylog/hylog.zep", 107 TSRMLS_CC);
			ZEPHIR_MM_RESTORE();
			return;
		}
	} else {
		...
		if (!zephir_is_true(&_7$$5)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_InvalidArgumentException, "A valid path must be passed to store log files.", "hyph/hylog/hylog.zep", 111);
			return;
		}
	}
	ZEPHIR_INIT_VAR(&_8);
...
PHP_METHOD(Hyph_Hylog_Hylog, interpolate) {
	...
	zephir_is_iterable(&context, 0, "hyph/hylog/hylog.zep", 127);
